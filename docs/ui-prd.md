...
| 2. Confirm tenant + app name | Auto-populated tenant (from the id_token `tid` claim) and app name (default "M365 MCP Scanner") shown as read-only; one-click **Confirm and continue** with an **Edit** fallback that re-renders the validated two-field form. An optional **Sign in for Power Platform** button lets the operator pre-authenticate `Add-PowerAppsAccount` synchronously (~10 s under `st.spinner`) so Step 4 can skip its second browser pop; skipping the button is fine — Step 4 will sign in if needed. |

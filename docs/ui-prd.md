...
| 2. Confirm tenant + app name | Auto-populated tenant (from the id_token `tid` claim) and app name (default "M365 MCP Scanner") shown as read-only; one-click **Confirm and continue** with an **Edit** fallback that re-renders the validated two-field form. |
...
| 4. PP Management App | Run `Add-PowerAppsAccount` + `New-PowerAppManagementApp` in a single pwsh subprocess — the operator sees one sign-in prompt for both cmdlets; auto-advance on confirmed success; Manual fallback expander preserves the copy-paste + Re-check flow (see ADR-0001 update) |
